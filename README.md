...
>
> To make the mod work correctly, open the `config/paper-global.yml` file and locate the `unsupported-settings` section.
>
> Change:
>
> ```yml
...
>
> This limitation is caused by Paper-specific behavior. Once this setting is changed, the mod should function correctly.
...
* A free or “safe” hotbar slot
   ...
   _A safe slot is a slot where the item has no usage delay (so swords, pickaxes, etc. do not qualify)._

If these conditions are not met, the mod simply does nothing.
